...
import { createWafacash, fileToBase64, type LoggedUser } from "./api";
...
export function NewWafacashForm({ user }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const [cinRectoFile, cinVersoFile, photoLocalFile] = await Promise.all([
        compressImageFile(form.cinRecto),
        compressImageFile(form.cinVerso),
        compressImageFile(form.photoLocal),
      ]);
      for (const f of [cinRectoFile, cinVersoFile, photoLocalFile]) {
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-md ring-1 ring-black/5 sm:p-8">
      <header className="mb-6 border-b border-[#C8D0C4] pb-4">
        <h2 className="text-2xl font-bold text-[#003C18]">Wafacash</h2>
      ...
      <form onSubmit={submit} className="space-y-5">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div>
            <label className={labelCls}>
              <span>Téléphone <span className="text-[#2F9E32]">*</span></span>
          ...
          <div className="sm:col-span-2">
            <label className={labelCls}>
              <span>Adresse <span className="text-[#2F9E32]">*</span></span>
              <span dir="rtl" className="text-xs font-normal text-[#00562B]/70">العنوان</span>
            </label>
            <input
              type="text"
              required
