...
import { getWePayPro, type WePayProRow, type LoggedUser } from "./api";
...
export function MyWePayProTable({ user }: Props) {
  const [rows, setRows] = useState<WePayProRow[]>([]);
  ...
  const commercials = [...new Set(rows.map((r) => r.created_by_username).filter(Boolean))].sort();
  const normalizedSearch = searchTerm.trim().toLowerCase();
  const sortedRows = [...rows].reverse();
  const filteredRows = sortedRows.filter((row) => {
    if (isAdmin && filterDateFrom && datePart(row.created_at) < filterDateFrom) return false;
    if (isAdmin && filterDateTo && datePart(row.created_at) > filterDateTo) return false;
    if (isAdmin && filterCommercial && row.created_by_username !== filterCommercial) return false;
    if (!normalizedSearch) return true;
    return (
      String(row.dossier_number || "").toLowerCase().includes(normalizedSearch) ||
      String(row.nom || "").toLowerCase().includes(normalizedSearch) ||
      String(row.prenom || "").toLowerCase().includes(normalizedSearch) ||
      String(row.cin || "").toLowerCase().includes(normalizedSearch) ||
      String(row.telephone || "").toLowerCase().includes(normalizedSearch) ||
      String(row.ville || "").toLowerCase().includes(normalizedSearch)
    );
  });
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getWePayPro(user);
      if (res.success && res.wepaypro) {
        setRows(res.wepaypro);
      } else {
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-md ring-1 ring-black/5 sm:p-8">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-3 border-b border-[#C8D0C4] pb-4">
        <div>
          <h2 className="text-2xl font-bold text-[#003C18]">
            {isAdmin ? "Tous les We Pay Pro" : "Mes We Pay Pro"}
          </h2>
          <p className="mt-1 text-sm text-[#00562B]/80">
            {isAdmin
              ? "Tous les dossiers We Pay Pro enregistrés par tous les commerciaux."
              : "Liste des dossiers We Pay Pro que vous avez enregistrés."}
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              placeholder="Rechercher par dossier, nom, prénom, CIN, téléphone ou ville..."
          ...
          {normalizedSearch && (
            <p className="text-xs text-[#00562B]/70">
              {filteredRows.length} résultat{filteredRows.length === 1 ? "" : "s"} trouvé
      ...
      {loading ? (
        <p className="py-12 text-center text-sm text-[#00562B]/70">Chargement des dossiers...</p>
      ) : rows.length === 0 ? (
        <p className="py-12 text-center text-sm text-[#00562B]/70">
          Aucun dossier We Pay Pro enregistré pour le moment.
        </p>
        ...
        <div className="overflow-x-auto">
          <table className={`w-full border-collapse text-sm ${isAdmin ? "min-w-[1250px]" : "min-w-[1100px]"}`}>
            ...
            <tbody>
              {filteredRows.map((r) => (
                <tr
                  key={r.dossier_number + r.created_at}
                  className="border-b border-[#C8D0C4]/60 text-[#003C18] transition hover:bg-[#B7F000]/10"
                >
                  <td className="px-3 py-3 whitespace-nowrap">{r.created_at}</td>
                  {isAdmin && <td className="px-3 py-3 text-[#00562B]/80">{r.created_by_username}</td>}
                  <td className="px-3 py-3 font-medium">{r.dossier_number}</td>
                  <td className="px-3 py-3">{r.nom}</td>
                  <td className="px-3 py-3">{r.prenom}</td>
                  <td className="px-3 py-3">{r.cin}</td>
                  <td className="px-3 py-3">{r.ville}</td>
                  <td className="px-3 py-3 whitespace-nowrap">{r.telephone}</td>
                  <td className="px-3 py-3">{r.operator}</td>
                  <td className="px-3 py-3">
                    <button
                      type="button"
                      className={linkBtn}
                      onClick={() => setPreview({ title: "CIN Recto", url: r.cin_recto_url })}
                    >
                      Voir Recto
                    </button>
                  </td>
                  <td className="px-3 py-3">
                    <button
                      type="button"
                      className={linkBtn}
                      onClick={() => setPreview({ title: "CIN Verso", url: r.cin_verso_url })}
                    >
                      Voir Verso
                    </button>
                  </td>
                  <td className="px-3 py-3">
                    <button
                      type="button"
                      className={linkBtn}
                      onClick={() => setPreview({ title: "Pièce jointe", url: r.piece_jointe_url })}
                    >
                      Voir Pièce
                    </button>
                  </td>
