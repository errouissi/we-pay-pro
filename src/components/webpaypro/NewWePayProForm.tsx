...
import { createWePayPro, fileToBase64, type LoggedUser } from "./api";
...
export function NewWePayProForm({ user }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!form.numeroDossier.trim()) {
      setFeedback({ type: "error", msg: "Veuillez saisir le numéro de dossier." });
      return;
    }
    ...
    try {
      ...
      const res = await createWePayPro(user, {
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-md ring-1 ring-black/5 sm:p-8">
      <header className="mb-6 border-b border-[#C8D0C4] pb-4">
        <h2 className="text-2xl font-bold text-[#003C18]">We Pay Pro Form</h2>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            <label className={labelCls}>
              <span>Numéro de dossier <span className="text-[#2F9E32]">*</span></span>
              <span dir="rtl" className="text-xs font-normal text-[#00562B]/70">رقم الملف</span>
            </label>
            <div className="flex overflow-hidden rounded-lg border border-[#C8D0C4] bg-white focus-within:border-[#2F9E32] focus-within:ring-2 focus-within:ring-[#2F9E32]/30">
              <span className="flex items-center border-r border-[#C8D0C4] bg-[#C8D0C4]/20 px-3 text-sm font-semibold text-[#003C18]">
                DOS-
              </span>
              <input
                type="text"
                ...
                onChange={(e) =>
                  setField("numeroDossier", e.target.value.replace(/^DOS-?/i, "").trimStart())
                }
                className="w-full bg-white px-3 py-2.5 text-sm text-[#003C18] outline-none"
              />
            </div>
          </div>
          <div>
      ...
      <SuccessModal
        ...
        message="Votre demande We Pay Pro a été envoyée avec succès."
      />
    </div>
  );
}
