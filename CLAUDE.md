...
There is no first-party backend. `src/components/webpaypro/api.ts` posts JSON to a single Google Apps Script Web App URL (`GOOGLE_SCRIPT_URL`). All actions (`login`, `createClient`, `getClients`, `createAgent`, `getAgents`, `createWafacash`, `getWafacash`, `createWePayPro`, `getWePayPro`, `createUser`, `getUsers`) are dispatched by an `action` field in the body. `doPost` also still accepts the legacy `createCache` action name as an alias that routes to the same Wafacash handler, so any stale cached frontend bundle keeps working. Two constraints to respect:
